...
hidden_type LDA_hidden(const DocsWords & docs_words, int max) {
    hidden_type hidden;
    for (int d = 0; d < docs_words.docs_number(); ++d) {
        std::vector<std::vector<std::pair<int, double> > > document;
        for (int w = 0; w < docs_words.unique_words_number(d); ++w) {
            document.push_back(std::vector<std::pair<int, double> >(
                                std::min(docs_words.word_counter(d, w), max),
                                std::pair<int, double>(0, 0))); // TODO: (-1, 0) --- test
        }
...
hidden_type LDA_hidden_mult(const DocsWords & docs_words, double gamma) {
    hidden_type hidden;
    for (int d = 0; d < docs_words.docs_number(); ++d) {
        std::vector<std::vector<std::pair<int, double> > > document;
        for (int w = 0; w < docs_words.unique_words_number(d); ++w) {
            document.push_back(std::vector<std::pair<int, double> >(
                                my_round(docs_words.word_counter(d, w) * gamma),
                                std::pair<int, double>(0, 0))); // TODO: (-1, 0) --- test
        }
        hidden.push_back(document);
    }
